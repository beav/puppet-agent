component "shellpath" do |pkg, settings, platform|
  ...
  pkg.install_file("./puppet-agent.sh", File.join(platform.srpm_only ? "/var/tmp/PUPPETBUILDROOT" : '', "/etc/profile.d/puppet-agent.sh"))
  ...
  if platform.name =~ /cisco-wrlinux-5/
    ...
    pkg.install_file("./puppet-agent.csh", File.join(platform.srpm_only ? "/var/tmp/PUPPETBUILDROOT" : '', "/etc/profile.d/puppet-agent.csh"))
  end
end
