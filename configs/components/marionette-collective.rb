component "marionette-collective" do |pkg, settings, platform|
  ...
  when "systemd"
    ...
    pkg.install_file "ext/aio/redhat/mcollective-systemd.logrotate", File.join(platform.srpm_only ? "/var/tmp/PUPPETBUILDROOT" : '', "/etc/logrotate.d/mcollective")
  when "sysv"
    if platform.is_deb?
    ...
    pkg.install_file "ext/aio/redhat/mcollective-sysv.logrotate", File.join(platform.srpm_only ? "/var/tmp/PUPPETBUILDROOT" : '', "/etc/logrotate.d/mcollective")
  when "launchd"
  ...
  pkg.install_file "ext/aio/redhat/mcollective-systemd.logrotate", File.join(platform.srpm_only ? "/var/tmp/PUPPETBUILDROOT" : '', "/etc/logrotate.d/mcollective")
  pkg.configfile File.join(platform.srpm_only ? "/var/tmp/PUPPETBUILDROOT" : "", "/etc/logrotate.d/mcollective") if platform.is_linux?
